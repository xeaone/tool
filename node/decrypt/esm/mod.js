...
export default async function decrypt(data, secret, options) {
    if (!data)
    ...
    const derived = await crypto.subtle.deriveKey({ name: 'PBKDF2', salt, iterations, hash }, imported, { name: algorithm, length }, true, ['decrypt']);
    const decrypted = await crypto.subtle.decrypt({ iv: vector, name: algorithm, length, tagLength: tag }, derived, body);
