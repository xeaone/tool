...
export default async function encrypt(data, secret, options) {
    if (!data)
    ...
    const imported = await crypto.subtle.importKey('raw', stringToBuffer(secret), { name: 'PBKDF2' }, false, ['deriveBits', 'deriveKey']);
    const derived = await crypto.subtle.deriveKey({ name: 'PBKDF2', salt, iterations, hash }, imported, { name: algorithm, length }, true, ['encrypt']);
    const encrypted = await crypto.subtle.encrypt({ iv: vector, name: algorithm, length, tagLength: tag }, derived, body);
